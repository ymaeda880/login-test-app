...
from lib.auth_session import (
    get_q_user, apply_q_user_to_session,
    debug_query_and_session, ensure_current_user
)
...
# 1) クエリ取得 → セッションへ（必要ならrerun）
q_user = get_q_user()
if q_user:
    apply_q_user_to_session(q_user, key="current_user", do_rerun=False)

# 2) デバッグ出力（任意）
debug_query_and_session(q_user)

# 3) 現在ユーザーの確定（UI込み）
#    - login_users: 例 {"maeda": {...}, "ym": {...}}
#    - users_db:    例 {"maeda": {"apps":[...]}, ...}  # ここは読み取り済みの dict を渡す
cur = ensure_current_user(
